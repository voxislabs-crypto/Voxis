...
function dampenForPrecision(value, neutral = 0.5, amount = 0.5) {
  return neutral + (value - neutral) * amount;
}

export function applyGenericProsodyText(text, envelope = {}, options = {}) {
  let output = String(text || "").trim();
  if (!output) {
    return output;
  }
  ...
  const phrasing = String(options?.phrasing || envelope?.phrasing || "balanced");
  const pauseSeconds = asNumber(options?.pauseSeconds, asNumber(envelope?.avgPauseSeconds, 0.28));
  const punctuation = String(options?.emphasisMode || "commas");
  ...
  output = applyWordLevelEmphasis(output, envelope?.emphasis?.words || [], { punctuation });
  return output.replace(/\s{2,}/g, " ").trim();
}

export function compileProsodyEnvelope({
  personality = {},
  mood = {},
  voiceProfile = {},
  directedText = "",
  ...
  const styleMode = String(speechContext?.styleMode || "performance").toLowerCase();
  ...
  if (precisionMode) {
    rateMultiplier = 1 + (rateMultiplier - 1) * 0.45;
  }
  ...
  const intensityBase = clamp(0.45 + (Math.abs(arousal) * 0.35) + (Math.max(0, dominance) * 0.2), 0, 1);
  const intensity = precisionMode ? dampenForPrecision(intensityBase, 0.5, 0.35) : intensityBase;
  const emphasisWords = precisionMode ? [] : collectEmphasisCandidates(personality, directedText);
  ...
  const elevenLabs = {
    ...
    emphasisMode: precisionMode ? "commas" : phrasing === "measured" ? "commas" : "ellipses",
  };
  ...
  const cloud = {
    speed: Number(targetRate.toFixed(3)),
    phrasing,
    preserveLiteralContent: precisionMode,
    instructions: precisionMode
      ? "Keep technical wording, hedging, and literal phrasing intact while speaking clearly."
      : phrasing === "bursty"
        ? "Deliver with energetic momentum and audible punch on emphasized words."
        : phrasing === "measured"
          ? "Deliver with deliberate pacing and clear pause boundaries."
          : "Deliver with natural cadence and light expressive emphasis.",
  };
  ...
  const piper = {
    lengthScale: Number(clamp(1 / targetRate, 0.55, 1.6).toFixed(3)),
    noiseScale: Number(clamp(0.48 + intensity * 0.32 + Math.max(0, arousal) * 0.14, 0.2, 1).toFixed(3)),
    noiseW: Number(clamp(0.54 + intensity * 0.24 + Math.max(0, arousal) * 0.1, 0.3, 1).toFixed(3)),
    phrasing,
    emphasisMode: precisionMode ? "commas" : phrasing === "bursty" ? "ellipses" : "commas",
  };
  ...
  return {
    ...
    source: {
      ...
      styleMode,
      contextCategory: String(speechContext?.category || "general").trim(),
    },
  };
}
...
export function applyProsodyToKokoroText(text, envelope = {}) {
  return applyGenericProsodyText(text, envelope, {
    phrasing: String(envelope?.provider?.kokoro?.phrasing || envelope?.phrasing || "balanced"),
    pauseSeconds: asNumber(envelope?.provider?.kokoro?.pauseSeconds, asNumber(envelope?.avgPauseSeconds, 0.28)),
    emphasisMode: String(envelope?.provider?.kokoro?.emphasisMode || "commas"),
  });
}
