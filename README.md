...
- Neural Core focus opens a readable side panel on explicit node click; memory focus resolves retrieval + extraction debug streams (`memoryRetrieved`, `memoryInjected`, `memoryExtracted`, `userMemoryRetrieved`, `userMemoryExtracted`) so Memory clicks work consistently across turn phases. The 3D renderer now uses an always-on cinematic network spin (plus camera orbit) and smoother organic pathway motion for a stronger "living brain" effect; focus/info panels were repositioned upward for better screen fit.
