...
- Neural Core focus opens a readable side panel on explicit node click; memory focus resolves retrieval + extraction debug streams (`memoryRetrieved`, `memoryInjected`, `memoryExtracted`, `userMemoryRetrieved`, `userMemoryExtracted`) so Memory clicks work consistently across turn phases. The 3D renderer now uses always-on cinematic network spin, smoother organic pathway motion, and per-neuron animated tendril fibers to create a stronger "living brain" network effect; focus/info panels were repositioned upward for better screen fit.
